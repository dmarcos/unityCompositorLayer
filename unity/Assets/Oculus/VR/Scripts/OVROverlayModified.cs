...
/// </summary>
public class OVROverlayModified : MonoBehaviour
{
#region Interface
	...
	public static OVROverlayModified[] instances = new OVROverlayModified[maxInstances];

#endregion

	public Material tex2DMaterial;
	public Material cubeMaterial;

	public string shaderName = "Oculus/Texture2D Blit";  // Added by Supermedium.
	...
	private bool SubmitLayer(bool overlay, bool headLocked, bool noDepthBufferTesting, OVRPose pose, Vector3 scale, int frameIndex)
	{
		...
		if (useExpensiveSuperSample)
		{
			Texture sc = layerTextures[0].swapChain[0];
			//Debug.Log("XXX SUBMIT LAYER " + layerDesc.TextureSize.w + " " + layerDesc.TextureSize.h);
		}
...
#region Unity Messages
	...
	void Awake()
	{
		...
		tex2DMaterial = new Material(Shader.Find(shaderName));
	...
	void OnEnable()
	{
		if (OVRManager.OVRManagerinitialized && !initialized)
		{
			InitOVROverlay();
			initialized = true;
		}
	...
	void LateUpdate()
	{
		...
		{
			Debug.Log("XXX Recreate Texture " + newDesc.TextureSize.w + " " + newDesc.TextureSize.h);
			DestroyLayerTextures();
			DestroyLayer();
		}
		...
		{
#else
		if (frameIndex > prevFrameIndex || forceUpdate)
		{
			forceUpdate = false;
#endif
